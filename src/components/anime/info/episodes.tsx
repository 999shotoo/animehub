...
export default async function AnimeInfoEps(id: any) {
    const episodes = await FetchEpisodesAnime(id.id);
    return (
        <>
            <div className="grid gap-4">
            <ScrollArea className="h-[40vh] md:h-[40vh] lg:h-[60vh] w-full rounded-md">
                {episodes.map((episode: any, index: any) => (
                    <Link href={`/anime/watch?id=${EpisodeNumberFormator(episode.id)}`}>
