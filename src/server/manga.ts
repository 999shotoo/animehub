...
export async function FetchTrendingManga(count: number){
    const response = await fetch(`${process.env.AnimeApi_1}/meta/anilist/advanced-search?type=MANGA&sort=["TRENDING_DESC"]&perPage=${count}`, { next: { revalidate: 3600 } });
